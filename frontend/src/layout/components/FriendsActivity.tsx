...
const FriendsActivity = () => {
	...
	return (
		<div className='h-full bg-zinc-900 rounded-lg flex flex-col'>
			<div className='p-3 sm:p-4 flex justify-between items-center border-b border-zinc-800'>
				<div className='flex items-center gap-2'>
					<Users className='size-4 sm:size-5 shrink-0' />
					<h2 className='font-semibold text-sm sm:text-base'>What they're listening to</h2>
				</div>
			</div>
								...
								<div className='flex items-start gap-2 sm:gap-3'>
									...
									<div className='flex-1 min-w-0'>
										<div className='flex items-center gap-1 sm:gap-2'>
											<span className='font-medium text-xs sm:text-sm text-white truncate'>{user.fullName}</span>
											{isPlaying && <Music className='size-3 sm:size-3.5 text-emerald-400 shrink-0' />}
										</div>

										{isPlaying ? (
											<div className='mt-1'>
												<div className='mt-1 text-xs sm:text-sm text-white font-medium truncate'>
